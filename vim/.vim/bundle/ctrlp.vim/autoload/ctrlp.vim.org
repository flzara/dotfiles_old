...
fu! s:AcceptSelection(action)
	...
	el
		...
		el
			le			let type = exttype == 'dict' ? exttype : 'list'
		en
	en
...
fu! s:OpenMulti(...)
	...
	" Commands for the rest of the f[&swb, '']
...
fu! s:remarksigns()
	...
	for ic in range(1, len(s:lines))
		let line = s:ispath ? fnamemodify(s! s:dosigns()
